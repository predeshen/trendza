...
function trendza_setup(): void {
    ...
    add_theme_support('responsive-embeds'); add_theme_support('woocommerce');
...
function trendza_render_product_card(int $product_id): void {
    if(!function_exists('wc_get_product'))return; $product=wc_get_product($product_id); if(!$product)return; $trend=trendza_get_product_trend($product_id);
    ?>
...
function trendza_render_intelligence_panel(): void {
    ...
    $quality=(float)$meta('_trendza_quality_score',0); $value=(float)$meta('_trendza_value_score',0); $summary=trim((string)$meta('_trendza_summary')); $ai=trim((string)$meta('_trendza_ai_summary'));
    $use_cases=$meta('_trendza_use_cases',[]); $pros=$meta('_trendza_pros',[]); $cons=$meta('_trendza_cons',[]); $specs=$meta('_trendza_specs',[]); $shipping=trim((string)$meta('_trendza_shipping_info'));
    foreach(['use_cases','pros','cons'] as $field){${$field}=is_string(${$field})?preg_split('/\r\n|\r|\n|,/',${$field},-1,PREG_SPLIT_NO_EMPTY):(array)${$field};}
    if(is_string($specs)){$decoded=json_decode($specs,true);$specs=is_array($decoded)?$decoded:[];} $has_content=$summary||$ai||$quality||$value||$use_cases||$pros||$cons||$specs||$shipping; if(!$has_content)return;
    ?>
    <section class="trendza-intelligence" aria-labelledby="trendza-intelligence-title"><div class="intelligence-heading"><div><span class="eyebrow">Trendza Intelligence</span><h2 id="trendza-intelligence-title">Why this product stands out</h2></div><?php if($trend):?><div class="intelligence-score"><strong><?php echo esc_html(number_format_i18n($trend['score'],0));?></strong><span>Trend score</span></div><?php endif;?></div>
    ...
    <?php if($quality||$value):?><div class="score-grid"><?php if($quality):?><div class="score-card"><span>Product quality</span><strong><?php echo esc_html(number_format_i18n($quality,0));?><small>/100</small></strong><div class="score-track"><i style="width:<?php echo esc_attr(min(100,$quality));?>%"></i></div></div><?php endif;?><?php if($value):?><div class="score-card"><span>Value score</span><strong><?php echo esc_html(number_format_i18n($value,0));?><small>/100</small></strong><div class="score-track"><i style="width:<?php echo esc_attr(min(100,$value));?>%"></i></div></div><?php endif;?></div><?php endif;?>
    <div class="intelligence-grid"><?php if($use_cases):?><div class="intel-card"><h3>Best for</h3><ul><?php foreach($use_cases as $item):?><li><?php echo esc_html(trim((string)$item));?></li><?php endforeach;?></ul></div><?php endif;?><?php if($pros):?><div class="intel-card"><h3>Pros</h3><ul class="check-list"><?php foreach($pros as $item):?><li><?php echo esc_html(trim((string)$item));?></li><?php endforeach;?></ul></div><?php endif;?><?php if($cons):?><div class="intel-card"><h3>Things to consider</h3><ul><?php foreach($cons as $item):?><li><?php echo esc_html(trim((string)$item));?></li><?php endforeach;?></ul></div><?php endif;?><?php if($specs):?><div class="intel-card"><h3>Key specs</h3><dl><?php foreach($specs as $key=>$spec_value):if(is_array($spec_value))$spec_value=implode(', ',$spec_value);?><div><dt><?php echo esc_html(ucwords(str_replace(['_','-'],' ',(string)$key)));?></dt><dd><?php echo esc_html((string)$spec_value);?></dd></div><?php endforeach;?></dl></div><?php endif;?></div>
    <?php if($shipping):?><div class="shipping-note"><strong>Delivery</strong><span><?php echo esc_html($shipping);?></span></div><?php endif;?></section>
    <?php
}
add_action('woocommerce_after_single_product_summary','trendza_render_intelligence_panel',8);

function trendza_render_recommendations(): void { global $product; if(!$product)return; echo '<section class="trendza-recommendations" data-trendza-recommendations="'.esc_attr($product->get_id()).'" aria-labelledby="trendza-recommendations-title"><div class="section-head"><div><span class="eyebrow">Picked for you</span><h2 id="trendza-recommendations-title">You may also like</h2></div></div><div class="product-grid recommendations-grid" data-recommendation-grid><p class="muted recommendation-loading">Finding relevant alternatives…</p></div></section>'; }
add_action('woocommerce_after_single_product_summary','trendza_render_recommendations',20);

function trendza_discovery_routes(): void { add_rewrite_rule('^(trending|rising|best-value|quality-picks)/?$','index.php?trendza_discovery=$matches[1]','top'); add_rewrite_rule('^(trending|rising|best-value|quality-picks)/page/([0-9]+)/?$','index.php?trendza_discovery=$matches[1]&paged=$matches[2]','top'); }
...
function trendza_discovery_query(): WP_Query { $key=sanitize_key((string)get_query_var('trendza_discovery'));$args=['post_type'=>'product','post_status'=>'publish','posts_per_page'=>24,'paged'=>max(1,(int)get_query_var('paged')),'no_found_rows'=>false];if($key==='trending'||$key==='rising'){$args['meta_key']='_trendza_trend_score';$args['orderby']='meta_value_num';$args['order']='DESC';$args['meta_query']=[['key'=>'_trendza_trend_status','value'=>$key==='trending'?'trending':'rising']];}elseif($key==='best-value'){$args['meta_key']='_trendza_value_score';$args['orderby']='meta_value_num';$args['order']='DESC';$args['meta_query']=[['key'=>'_trendza_value_score','compare'=>'EXISTS']];}elseif($key==='quality-picks'){$args['meta_key']='_trendza_quality_score';$args['orderby']='meta_value_num';$args['order']='DESC';$args['meta_query']=[['key'=>'_trendza_quality_score','compare'=>'EXISTS']];}return new WP_Query($args); }
