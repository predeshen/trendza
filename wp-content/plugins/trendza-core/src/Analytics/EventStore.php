...
final class EventStore {
    ...
    public static function install(): void {
        ...
        dbDelta("CREATE TABLE {$table} (
            ...
            PRIMARY KEY (id),
            KEY product_event_time (product_id,event_type,occurred_at),
            KEY event_time (event_type,occurred_at),
            KEY session_time (session_hash,occurred_at)
        ) {$charset};");
    }

    public static function record(int $productId, string $eventType, string $sessionHash = '', array $metadata = []): bool {
        global $wpdb;
        $allowed = ['view','search','add_to_cart','begin_checkout','purchase'];
        if (!in_array($eventType, $allowed, true)) return false;
        return false !== $wpdb->insert(self::table(), [
            'product_id' => max(0, $productId),
            'event_type' => $eventType,
            'occurred_at' => current_time('mysql', true),
            'session_hash' => substr(hash('sha256', $sessionHash), 0, 64),
            'metadata' => $metadata ? wp_json_encode($metadata) : null,
        ], ['%d','%s','%s','%s','%s']);
    }

    public static function count(int $productId, string $eventType, int $hours): int {
        global $wpdb;
        $since = gmdate('Y-m-d H:i:s', time() - max(1, $hours) * HOUR_IN_SECONDS);
        return (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM " . self::table() . " WHERE product_id=%d AND event_type=%s AND occurred_at >= %s",
            $productId, $eventType, $since
        ));
    }
    ...
    public static function prune(int $days = 90): void {
        ...
        $wpdb->query($wpdb->prepare('DELETE FROM ' . self::table() . ' WHERE occurred_at < %s', $before));
    }
}
